...
@torch.no_grad()
def get_act_scales(model, tokenizer, dataset, num_samples=128, seq_len=2048):
    model.eval()
    ...
    for i in tqdm(range(num_samples)):
        ...
        if 'text' in d:
            input_ids = tokenizer(
                d["text"], return_tensors="pt", max_length=seq_len, truncation=True
            ).input_ids.to(device)
        else:
...
def smooth_ln_fcs(ln, fcs, act_scales, alpha=0.5):
    ...
    # fc.weight.data = quantize_dequantize(fc.weight)
...
def main():
    ...
    dataset = load_dataset("lambada", split=f"validation[:{args.num_samples}]").shuffle(seed=1)
