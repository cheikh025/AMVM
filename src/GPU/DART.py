...
class DART():
    ...
    def boundary_pixels(self, img):
        ...
        bool_mask = np.full(fill_value=False, shape=img.shape[:2], dtype=np.bool_)
    ...
    def free_pixels(self):
        ...
        free_pixels = np.random.choice(a=self.c, 
                                    size=self.rec_shape, 
                                    p=self.probs).astype(np.bool_)
        return free_pixels

def SIRT(vol_geom, vol_data, projector_id, sino_id, iters=1000, use_gpu=False):
    ...
    alg_cfg['option'] = {'MinConstraint': 0, 'MaxConstraint': 255}
...
def SART(vol_geom, vol_data, projector_id, sino_id, iters=1000, use_gpu=False):
    rec_id = astra.data2d.create('-vol', vol_geom, data=vol_data)
